...
[News](#news) · [快速开始](#-快速开始) · [适合谁](#-它适合谁) · [解决什么问题](#-它解决什么问题) · [功能亮点](#-功能亮点) · [支持公司](#-支持公司)  · [真实 Demo](#-真实-demo-案例) · [输出示例](#-输出示例) · [FAQ](#-faq) · [更新计划](#-更新计划)
...
- **2026-06-04**：新增网页版 Mock Interview UI，支持中英文切换、JD/简历文件或文本输入、自选模型供应商与 API Key、基于回答的多轮追问演练。
...
1. 在左侧填写目标公司、目标岗位和面试轮次。
2. 在中间输入 JD，可以粘贴链接、粘贴文本，或上传 PDF、Word、TXT、Markdown 文件。
3. 在简历区域默认使用「文件」上传，也可以切换到「文本」粘贴简历内容。
4. 在左下角选择模型供应商和模型，API Key 输入框直接填写以 `sk-` 开头的 Key；不填写时会使用本地模拟，填写后会调用所选供应商生成真实问题和追问。
5. 点击「开始模拟面试」后，在右侧回答当前问题；点击「生成追问」后，页面会基于你的回答继续追问，并允许继续填写追问答案、进一步追问。
6. 右上角流程状态会随着输入材料、生成面试、追问演练自动变化；右上角也可以切换 English / 中文界面。

<img src="assets/mock-interview-ui.png" alt="网页版 Mock Interview UI 截图" width="100%">
...
常见问题参考 [FAQ.md](FAQ.md)。如有其他问题，欢迎提 issue。
